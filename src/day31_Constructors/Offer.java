package day31_Constructors;

public class Offer {

    public String location, companyName, jopTitle;
    public double salary;
    public boolean hasBenefit, hasPTO, isMFH, isFullTime;


    public void setInfo(String location, String companyName, String jopTitle, double salary, boolean hasBenefit, boolean hasPTO, boolean isMFH, boolean isFullTime) {
        this.location = location;
        this.companyName = companyName;
        this.jopTitle = jopTitle;
        this.salary = salary;
        this.hasBenefit = hasBenefit;
        this.hasPTO = hasPTO;
        this.isMFH = isMFH;
        this.isFullTime = isFullTime;
    }


    public String toString() {
        return "Offer{" +
                "location='" + location + '\'' +
                ", companyName='" + companyName + '\'' +
                ", jopTitle='" + jopTitle + '\'' +
                ", salary=" + salary +
                ", hasBenefit=" + hasBenefit +
                ", hasPTO=" + hasPTO +
                ", isMFH=" + isMFH +
                ", isFullTime=" + isFullTime +
                '}';
    }

}
/*1. Create a custom class named Offer
            Attributes:
                    location, companyName, jobTitle, salary, hasBenefit, hasPTO, isWFH, isFullTime

            Actions:
                setInfo(): sets all the instance variables
                toString(): returns the full info of the Offer Object */

